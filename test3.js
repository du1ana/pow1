...
async function pow_argon2(lgrhex, pubkeyhex, sevens)
{
    const t0 = performance.now();
	for(let upto = 0n; upto < 0xFFFFFFFFFFFFFFFFn ; upto++)
	{
		...
		//let sha = argon2.hash(buf);
        //let sha = await argonHash(buf);
        ...
        await argon2.hash(buf,options)
        .then(hash => {
            sha=hash.substring(31);
        })
        .catch(err => {
...
async function main() {
    ...
    //await pow_sha512(lgrhex, pubkeyhex, 5)
    await pow_argon2(lgrhex, pubkeyhex, 2)
